...
export const classQueries = {
  ...
  async query(
    ...
    if (options.names && options.names.length > 0) {
      ...
      const valuesList = options.names
        .map((_, idx) => `($${paramIndex + idx * 2}, $${paramIndex + idx * 2 + 1})`)
        .join(", ");
...
export function createClassLoaders(client: pg.Client | pg.PoolClient) {
  ...
  >(async (keys) => {
    ...
    return keys.map((key) => {
      const lookupKey = key.relkind ? `${key.namespaceOid}:${key.relkind}` : `${key.namespaceOid}`;
