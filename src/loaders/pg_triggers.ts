import { PgTrigger, PgTriggerSchema } from "../types.js";
import pg from "pg";
...
export const triggerQueries = {
   ...
   */
  async query(client: pg.Client | pg.PoolClient, options: TriggerQueryOptions): Promise<PgTrigger[]> {
    let query = `
      SELECT
        t.oid,
        t.tgname,
...
export function createTriggerLoaders(client: pg.Client | pg.PoolClient) {
  ...
  const triggersByRelationLoader = new DataLoader<number, PgTrigger[]>(async (relationOids) => {
    ...
    const triggers = await triggerQueries.query(client, { triggerRelids: uniqueOids });
